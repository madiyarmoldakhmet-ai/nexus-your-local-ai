...
const ChatWindow = () => {
  const { messages, selectedChatId, chats } = useChat();
  ...
  const title = chat?.isGroup ? chat.name : chat?.participants?.find((uid) => uid !== chat?.currentUserUid);
  ...
  return (
    <div className="chat-window" style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
      <header style={{ padding: '0.5rem', borderBottom: '1px solid #e0e0e0', background: '#fafafa' }}>
        <h4 style={{ margin: 0 }}>{title || 'Chat'}</h4>
      </header>
      <div
        ref={containerRef}
        className="messages"
      ...
      >
        {messages.map((msg) => (
          <div
            ...
            style={{
              marginBottom: '0.5rem',
              textAlign: msg.senderId === chat?.currentUserUid ? 'right' : 'left',
            }}
          >
            <span
              style={{
                ...
                borderRadius: '0.8rem',
                background: msg.senderId === chat?.currentUserUid ? '#e6f7ff' : '#f0f0f0',
                maxWidth: '70%',
